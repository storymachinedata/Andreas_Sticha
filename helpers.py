...
filters = {
    "Total Interaction: High to Low": ["Total Interactions", False],
...
a2 = [
    ...
    "Tankgutscheine",
    "Tankgutschein",
    "Steuerliche Optimierung für Arbeitgeber",
    "Steuerliche Optimierung für Arbeitgebende",
    "Steuerliche Optimierung für Arbeitgeber:innen",
]
a3 = [
    "HR",
    "Human Resources",
...
a6 = [
    ...
    "Steuerliche Freibeträge für Arbeitgeber",
]
...
def find_matched_keywords(text, keywords2match=None):

    print(keywords2match)
    if keywords2match in text:
        return True
    return False
...
def display_filters():
    ...
    with col1:
        filter_day = st.number_input(
            "How many days older posts?", min_value=1, max_value=100, value=7, step=1
        )
        if filter_day:
    ...
    with col3:
        filter_date = st.selectbox(
            "Filter by total Post date", ("Posts: Newest First", "Posts: Oldest First")
        )
...
def choice_selector(dataframe, choice="All"):
    ...
    if choice == "All":
        ...
        dataframe["MatchedKeywords"] = dataframe["textContent"].apply(
            find_matched_keywords, keywords2match=choice
        )
        selected_df = dataframe.loc[dataframe["MatchedKeywords"] == True]
        return selected_df
